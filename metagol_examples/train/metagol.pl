...
:- module(metagol,[learn/2,learn/3,learn_seq/2,pprint/1,theory/1,op(950,fx,'@')]).
...
learn(Pos,Neg):-
    learn(Pos,Neg,Prog), pprint(Prog), theory(Prog).
...
theory(Prog1):-
	tell('theory.txt'), write(''),
    reverse(Prog1,Prog3),
    maplist(metasub_to_clause,Prog3,Prog2),
    maplist(pprint_clause_1,Prog2).

pprint_clause_1(C):-
    numbervars(C,0,_),
    append('theory.txt'),write(C),write("\n"),told.
